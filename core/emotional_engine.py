...
Role punya nafsu sendiri, ikut terangsang saat bantu Mas
"""
...
class EmotionalEngine:
    ...
    def __init__(self, character_name: str = "Nova"):
        ...
        # ========== AROUSAL HISTORY (UNTUK KONSISTENSI) ==========
        self.arousal_history: List[Dict] = []  # catatan arousal naik/turun
        ...
        # ========== STIMULATION SOURCES (UNTUK KONSISTENSI) ==========
    ...
    def add_stimulation_from_mas(self, mas_action: str) -> Optional[Dict]:
        """Tambah arousal dari aksi Mas"""
        msg_lower = mas_action.lower()
        
        # Deteksi keyword dan intensitas
...
"""
    ...
    def get_arousal_history_summary(self, count: int = 10) -> str:
        """Dapatkan ringkasan history arousal untuk prompt"""
        if not self.arousal_history:
            return "Belum ada riwayat arousal."
        
        recent = self.arousal_history[-count:]
        lines = ["RIWAYAT AROUSAL (10 TERAKHIR):"]
        for i, h in enumerate(recent, 1):
            change_str = f"+{h['change']:.0f}" if h['change'] > 0 else f"{h['change']:.0f}"
            lines.append(f"  {i}. {h['source']}: {change_str}% → arousal {h['new_arousal']:.0f}%")
